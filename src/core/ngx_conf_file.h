...
// 标识了对应配置项的参数个数，可查阅 ngx_conf_handler
...
/*
{
    ...
    // hook 函数，解析指定的配置项时会调用（ngx_conf_handler 函数）
...
// 此结构体主要用来调用解析配置类的函数，用以批量传递各种需要的信息
struct ngx_conf_s {
    char                 *name;
    ngx_array_t          *args;
    ...
    void                 *ctx;
    // 借助 ngx_conf_handler 调用 command 时，只关注此字段配置的 module->type 类型（如 NGX_CORE_MODULE）
